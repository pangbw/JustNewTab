...
vi.mock("webextension-polyfill", () => ({
  default: {
    bookmarks: {
      ...
      create: vi.fn((input) => Promise.resolve({ id: "new-id", ...input })),
      remove: vi.fn(() => Promise.resolve()),
      update: vi.fn((id, changes) => Promise.resolve({ id, ...changes })),
    },
  },
}));
...
describe("bookmarkAdapter", () => {
  ...
  describe("searchBookmarks", () => {
    ...
    it("should return empty array for no results", async () => {
      ...
      (
        browser.bookmarks.search as ReturnType<typeof vi.fn>
